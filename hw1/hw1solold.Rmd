...
1. Apply for the [Student Developer Pack](https://education.github.com/pack) at GitHub using your UCLA email.
    ...
    I have applied for the Student Developer Pack at GitHub using my UCLA email.
...
2. Create a **private** repository `biostat-m280-2018-winter` and add `Hua-Zhou` and `juhkim111` as your collaborators with write permission.
    ...
    I created a private repository `biostat-m280-2018-winter` and added `Hua-Zhou` and `juhkim111` as my collaborators with write permission.
...
3. Top directories of the repository should be `hw1`, `hw2`, ... Maintain two branches `master` and `develop`. The `develop` branch will be your main playground, the place where you develop solution (code) to homework problems and write up report. The `master` branch will be your presentation area. Submit your homework files (R markdown file `Rmd`, `html` file converted from R markdown, all code and data sets to reproduce results) in `master` branch.
    ...
    I maintained the two branches `master` and `develop` and created the repostory `hw1`.
...
-   Number of SNPs in each chromosome:
    ...
    | 21         | 239352         |
    ```{bash, echo=TRUE, eval=TRUE}
    awk '{print $1}' /home/m280-data/hw1/merge-geno.bim | sort | uniq -c
    ```
...
4. Statistical geneticists often have to reformat a data set to feed into various analysis programs. For example, to use the Mendel software <http://www.genetics.ucla.edu/software/mendel>, we have to reformat the data set to be read by Mendel.
    ...
    ```{bash, echo=TRUE, eval=TRUE}
    awk '{OFS = ","} BEGIN {print "    2.40 = FILE FORMAT VERSION NUMBER.\n"  \
    "8348674 = NUMBER OF SNPS LISTED HERE."} {print $1, $2, $4}'   \
    /home/m280-data/hw1/merge-geno.bim > mendel_snp.txt
    head -10 mendel_snp.txt
    ```
    ...
    /home/m280-data/hw1/merge-geno.fam | awk -F, -v sex="" '{OFS = ","}   \
    {if ($5 == 1) sex="M"; else sex="F"} {print $1, $2, $3, $4, sex, $6}'   \
    |sed 's/T2DG//g' | awk -F, '{OFS = ","} {for (i = 1; i <= NF; i++)   \
    {$i = ($i == 0 ? "": $i)}; print}' > mendel_ped.txt
...
for both methods.
    ...
    I expanded the `runSim.R` script with the arguments and renamed it as 'runSim_hw1.R`.  \
    The "1 10 gaussian 10" here is for testing purpose only.
    ```{bash, echo=TRUE, eval=TRUE}
      Rscript runSim_hw1.R 1 10 gaussian 10
    ```
...
2. Modify the [`autoSim.R`](http://hua-zhou.github.io/teaching/biostatm280-2018winter/slides/02-linux/autoSim.R) script to run simulations with combinations of sample sizes `nVals = seq(100, 500, by=100)` and distributions `distTypes = c("gaussian", "t1", "t5")` and write output to appropriately named files. Use `rep = 50`, and `seed = 280`. 
    ...
    I modified the `autoSim.R` script to run the simulations and remamed it as `autoSim_hw1.R`. I wrote output to appropriately named files. 
    ```{bash, echo=TRUE, eval=TRUE}
    Rscript autoSim_hw1.R
    ```

3. Write an R script to collect simulation results from output files and print average MSEs in a table of format

    I wrote an R script called `table_hw1.R` to collect simulation results from output files and printed average MSEs in a table of format.
    ```{bash, echo=TRUE, eval=TRUE}
    Rscript table_hw1.R
    ```
...
|:---|:--------|----------:|---------:|---------:|
|100 |PrimeAvg | 2808.42600| 0.0664284| 0.0414674|
|    |SampAvg  |  230.23720| 0.0167508| 0.0094834|
|200 |PrimeAvg |  799.31390| 0.0418479| 0.0187544|
|    |SampAvg  |   95.87976| 0.0076665| 0.0071637|
|300 |PrimeAvg |  446.40240| 0.0241224| 0.0200254|
