...
busydo <- left_join(airports_tbl, do, by = c("faa" = "port"), copy = TRUE)

#busyp <- do %>%
  #left_join(airports_tbl, by = c("port" = "faa"), copy = TRUE)
  
  ggplot() + 
  geom_polygon(data = states, aes(x = long, 
    y = lat, fill = region, group = group), color = "white") + 
  geom_point(data = bap, aes(x = long, y = lat), color = "yellow", size = 4) +
  coord_fixed(1.3) +
  guides(fill=FALSE) 
```
